  ...
  try {
    ...
    if (selectedPhotos.length > 0) {
      ...
      for (const photo of selectedPhotos) {
        if (yPos > pageHeight - margin - 50) {
          // Check space for image, approx 50mm
          doc.addPage();
          yPos = margin;
        }
        try {
          ...
          photo.compressedDataUrl = imgDataUrl;
    ...
    if (
      ...
      try {
        ...
        if (!shared) {
          // Only show this if share truly failed, not if user cancelled
          updateStatusMessage(
            "pdfGenerateStatus",
            "Share failed or cancelled. Downloading.",
            "error",
            5000
          );
        }
    ...
    doc.save(fileName); // Always download
    if (!shared) {
      // If not shared, ensure download message is shown
      updateStatusMessage(
        "pdfGenerateStatus",
        "PDF downloaded!",
        "success",
    ...
    // Status message timeout is handled by updateStatusMessage
  }
}
