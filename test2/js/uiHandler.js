...
function applyStateToFields(state) {
  ...
  for (const id in state.fields) {
    ...
    if (element) {
      if (element.type === "checkbox") {
        element.checked = state.fields[id];
      } else {
        element.value = state.fields[id];
      }
...
function updateWarehouseHighlight() {
  const warehouseEl = document.getElementById("sheetWarehouse");
  if (!warehouseEl) return;

  if (warehouseEl.value.toLowerCase().startsWith("masp")) {
    warehouseEl.classList.add("warehouse-masp-attention");
  } else {
    warehouseEl.classList.remove("warehouse-masp-attention");
  }
}
...
function createQuestionFields() {
  ...
  questions.forEach((q) => {
    ...
    select.addEventListener("change", handleNoSelection);
